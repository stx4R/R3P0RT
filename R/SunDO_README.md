...
| 배포 | Firebase Hosting · 보안 헤더 | 앱과 인증 핸들러를 같은 출처에 둔다. X-Frame-Options·CSP `frame-ancestors` 등 적용 |
